...
use sway_groups_tests::common::{
    db_query, get_focused_workspace, orig_active_group, output_contains, swayg_fixture_db, swayg_output,
    window_count_in_tree, workspace_count_in_sway, workspace_of_window, DummyWindowHandle,
    TestFixture,
};

const GROUP_A: &str = "zz_test_group_a__05e";
const GROUP_B: &str = "zz_test_group_b__05e";
const WS1: &str = "zz_test_ws1__05e";
...
#[tokio::test]
async fn test_05e_multi_group_unglobal() {
    ...
    assert_eq!(ws1_wsgrp, "0", "{} has 0 workspace_groups entries", WS1);
    ...
    assert_eq!(ws1_not_global, "0", "{} is no longer global", WS1);

    // --- Verify: WS1 in Group B only (NOT Group A) ---
    let ws1_wsgrp_count: String = db_query(
        &fixture.db_path,
        &format!(
            "SELECT count(*) FROM workspace_groups wg JOIN workspaces w ON w.id = wg.workspace_id WHERE w.name = '{}'",
            WS1
    ...
    assert_eq!(
        ws1_wsgrp_count, "1",
        "{} has exactly 1 workspace_groups entry",
        WS1
    );
    ...
    // --- Verify: Group A was NOT resurrected ---
    assert_eq!(
        db_count(&fixture.db_path, "groups", "name", GROUP_A),
        0,
        "{} still deleted (not resurrected by unglobal)",
        GROUP_A
    );
