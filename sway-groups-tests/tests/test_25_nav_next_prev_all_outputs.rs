use sway_groups_tests::common::{
    DummyWindowHandle, TestFixture, create_virtual_output, db_count, get_focused_workspace,
    orig_active_group, unplug_output,
};
...
async fn test_25_nav_next_prev_all_outputs() {
    ...
    assert_eq!(
        ...
        WS1,
        "next single-output wraps to WS1 (only WS1 visible)"
    );
    ...
    assert_eq!(
        get_focused_workspace().unwrap(),
        WS2,
        "next --all-outputs → WS2 (cross-output)"
    );
    ...
    assert_eq!(
        get_focused_workspace().unwrap(),
        WS2,
        "next --all-outputs → WS2"
    );

    fixture
        .swayg(&["nav", "next", "--all-outputs", "--wrap"])
        .success();
    assert_eq!(
        get_focused_workspace().unwrap(),
        WS1,
        "next --all-outputs --wrap → WS1"
    );

    // --- nav prev --all-outputs --wrap from WS1 wraps to WS2 ---
    fixture
        .swayg(&["nav", "prev", "--all-outputs", "--wrap"])
        .success();
    assert_eq!(
        get_focused_workspace().unwrap(),
        WS2,
        "prev --all-outputs --wrap → WS2"
    );
    ...
    for output in [&fixture.orig_output, &virt_output] {
        fixture
            .swayg(&["group", "select", GROUP, "--output", output])
            .success();
        fixture
            .swayg(&[
                "group",
