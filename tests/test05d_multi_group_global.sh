#!/bin/bash
# Test: workspace global on multi-group workspace.
...
echo -e "\033[1m=== Test: workspace global on multi-group workspace ===\033[0m"
...
# --- 5. Switch to Group B, add WS1 to Group B ---
...
if [ "$WS1_WSGRP" = "0" ]; then pass "$WS1 has no workspace_groups entries"; else fail "$WS1 has no workspace_groups entries (count=$WS1_WSGRP)"; fi

OUT_VISIBLE=$($SG workspace list --visible --plain --output $OUTPUT 2>/dev/null)
echo "$OUT_VISIBLE" | grep -q "$WS1"
if [ $? -eq 0 ]; then pass "$WS1 is visible (global)"; else fail "$WS1 is visible (global)"; fi
...
if [ $? -eq 0 ]; then pass "$WS1 visible in Group B (global)"; else fail "$WS1 visible in Group B (global)"; fi

# --- 7. Switch back to original group ---
