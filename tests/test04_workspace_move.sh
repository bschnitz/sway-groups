...
echo -e "\033[1m=== Test: move workspace between groups ===\033[0m"
...
if [ "$GA_COUNT" = "0" ]; then
    pass "$GROUP_A does not exist in DB"
else
    fail "$GROUP_A must not exist in DB (count=$GA_COUNT)"
...
if [ "$GB_COUNT" = "0" ]; then
    pass "$GROUP_B does not exist in DB"
else
    fail "$GROUP_B must not exist in DB (count=$GB_COUNT)"
    echo "Results: $PASS passed, $FAIL failed (ABORTED)"
    exit 1
fi
...
# --- 4. Create group A and add workspace ---
run "Create group A" \
    '$SG group select $OUTPUT '"'$GROUP_A'"' --create 2>&1'

GA_EXISTS=$(sqlite3 "$DB" "SELECT count(*) FROM groups WHERE name = '$GROUP_A';")
if [ "$GA_EXISTS" = "1" ]; then
    pass "group '$GROUP_A' was created"
else
    fail "group '$GROUP_A' was created (count=$GA_EXISTS)"
fi
...
if [ "$WS1_APPID" -ge 1 ]; then
    pass "kitty with app_id '$WS1' is running"
else
...
if [ "$WS1_IN_GA" = "1" ]; then
    pass "$WS1 is in group '$GROUP_A'"
else
    fail "$WS1 is in group '$GROUP_A' (count=$WS1_IN_GA)"
fi

# --- 5. Move workspace to group B ---
run "Move $WS1 to group '$GROUP_B'" \
    '$SG workspace move "'$WS1'" --groups "'$GROUP_B'" 2>&1'
...
if [ "$WS1_IN_GB" = "1" ]; then
    pass "$WS1 is now in group '$GROUP_B'"
else
    fail "$WS1 is now in group '$GROUP_B' (count=$WS1_IN_GB)"
fi
...
if [ "$WS1_NOT_IN_GA" = "0" ]; then
    pass "$WS1 is no longer in group '$GROUP_A'"
else
    fail "$WS1 is no longer in group '$GROUP_A' (count=$WS1_NOT_IN_GA)"
fi

# --- 6. Switch to group B (leaves group A) ---
run "Switch to group B (should auto-delete group A)" \
    '$SG group select $OUTPUT '"'$GROUP_B'"' --create 2>&1'

GA_GONE=$(sqlite3 "$DB" "SELECT count(*) FROM groups WHERE name = '$GROUP_A';")
if [ "$GA_GONE" = "0" ]; then
    pass "$GROUP_A auto-deleted (workspace moved away)"
else
    fail "$GROUP_A auto-deleted (workspace moved away) (count=$GA_GONE)"
fi

OUT=$($SG workspace list --visible --plain --output $OUTPUT 2>/dev/null)
echo "$OUT" | grep -q "$WS1"
if [ $? -eq 0 ]; then
    pass "$WS1 is visible in group '$GROUP_B'"
else
    fail "$WS1 is visible in group '$GROUP_B'"
fi

# --- 7. Switch to original group ---
...
if [ "$GB_STILL" = "1" ]; then
    pass "$GROUP_B NOT auto-deleted (still has workspaces)"
else
    fail "$GROUP_B NOT auto-deleted (still has workspaces) (count=$GB_STILL)"
fi

# --- 8. Kill kitty, switch to group B and back (should auto-delete) ---
run "Kill kitty $WS1" \
...
run "Switch back to original group (should auto-delete group B)" \
    '$SG group select $OUTPUT '"'$ORIG_GROUP'"' 2>&1'
...
if [ "$GB_GONE" = "0" ]; then
    pass "$GROUP_B auto-deleted"
else
    fail "$GROUP_B auto-deleted (count=$GB_GONE)"
fi
