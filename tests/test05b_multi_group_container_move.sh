#!/bin/bash
# Test: container move to workspace in another group.
...
echo -e "\033[1m=== Test: container move to workspace in another group ===\033[0m"
...
# --- 9. container move to WS1 (existing in Group A, not in Group B) ---
run "Move container to $WS1 (existing in Group A, adds to Group B)" \
    '$SG container move "'$WS1'" --switch-to-workspace 2>&1'

FOCUSED=$(swaymsg -t get_workspaces 2>/dev/null | jq -r '.[] | select(.focused) | .name')
if [ "$FOCUSED" = "$WS1" ]; then pass "focused on '$WS1'"; else fail "focused on '$WS1' (got '$FOCUSED')"; fi

WS2_ON_WS1=$(swaymsg -t get_tree 2>/dev/null | jq -r '[.. | objects | select(.type? == "workspace")] | .[] | {name: .name, apps: [.. | objects | select(.app_id? != null) | .app_id]} | select(.apps | index("'$WS2'")) | .name')
if [ "$WS2_ON_WS1" = "$WS1" ]; then pass "kitty '$WS2' is on workspace '$WS1'"; else fail "kitty '$WS2' is on workspace '$WS1' (got '$WS2_ON_WS1')"; fi
...
# --- 10. Verify visibility in Group B ---
...
# --- 11. Switch back to original group ---
