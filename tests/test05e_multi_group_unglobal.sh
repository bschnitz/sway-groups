#!/bin/bash
# Test: workspace unglobal on previously multi-group workspace.
...
echo -e "\033[1m=== Test: workspace unglobal on previously multi-group workspace ===\033[0m"
...
# --- 8. Set WS1 global (removes from both groups, auto-deletes Group A) ---
run "Set $WS1 global" \
    '$SG workspace global "'$WS1'" 2>&1'
WS1_GLOBAL=$(sqlite3 "$DB" "SELECT is_global FROM workspaces WHERE name = '$WS1';")
if [ "$WS1_GLOBAL" = "1" ]; then pass "$WS1 is global in DB"; else fail "$WS1 is global in DB (got '$WS1_GLOBAL')"; fi
WS1_WSGRP=$(sqlite3 "$DB" "SELECT count(*) FROM workspace_groups wg JOIN workspaces w ON w.id = wg.workspace_id WHERE w.name = '$WS1';")
if [ "$WS1_WSGRP" = "0" ]; then pass "$WS1 has 0 workspace_groups entries"; else fail "$WS1 has 0 workspace_groups entries (count=$WS1_WSGRP)"; fi
GA_GONE=$(sqlite3 "$DB" "SELECT count(*) FROM groups WHERE name = '$GROUP_A';")
if [ "$GA_GONE" = "0" ]; then pass "$GROUP_A auto-deleted (not active, no non-global workspaces)"; else fail "$GROUP_A auto-deleted (count=$GA_GONE)"; fi
GB_STILL=$(sqlite3 "$DB" "SELECT count(*) FROM groups WHERE name = '$GROUP_B';")
if [ "$GB_STILL" = "1" ]; then pass "$GROUP_B still exists (is active group)"; else fail "$GROUP_B still exists (count=$GB_STILL)"; fi

# --- 9. Unglobal WS1 ---
run "Unglobal $WS1" \
    '$SG workspace unglobal "'$WS1'" 2>&1'
WS1_NOT_GLOBAL=$(sqlite3 "$DB" "SELECT is_global FROM workspaces WHERE name = '$WS1';")
if [ "$WS1_NOT_GLOBAL" = "0" ]; then pass "$WS1 is no longer global"; else fail "$WS1 is no longer global (got '$WS1_NOT_GLOBAL')"; fi

# --- 10. Verify: WS1 in Group B only (NOT Group A) ---
WS1_WSGRP_COUNT=$(sqlite3 "$DB" "SELECT count(*) FROM workspace_groups wg JOIN workspaces w ON w.id = wg.workspace_id WHERE w.name = '$WS1';")
if [ "$WS1_WSGRP_COUNT" = "1" ]; then pass "$WS1 has exactly 1 workspace_groups entry"; else fail "$WS1 has exactly 1 workspace_groups entry (count=$WS1_WSGRP_COUNT)"; fi

WS1_IN_GB=$(sqlite3 "$DB" "SELECT count(*) FROM workspace_groups wg JOIN groups g ON g.id = wg.group_id JOIN workspaces w ON w.id = wg.workspace_id WHERE w.name = '$WS1' AND g.name = '$GROUP_B';")
if [ "$WS1_IN_GB" = "1" ]; then pass "$WS1 is in group '$GROUP_B'"; else fail "$WS1 is in group '$GROUP_B' (count=$WS1_IN_GB)"; fi

# --- 11. Verify: Group A was NOT resurrected ---
GA_STILL_GONE=$(sqlite3 "$DB" "SELECT count(*) FROM groups WHERE name = '$GROUP_A';")
if [ "$GA_STILL_GONE" = "0" ]; then pass "$GROUP_A still deleted (not resurrected by unglobal)"; else fail "$GROUP_A still deleted (count=$GA_STILL_GONE)"; fi

# --- 12. Verify: WS1 visible in Group B ---
...
# --- 13. Switch back to original group (Group B should NOT auto-delete, WS1 still in sway) ---
...
GB_STILL=$(sqlite3 "$DB" "SELECT count(*) FROM groups WHERE name = '$GROUP_B';")
if [ "$GB_STILL" = "1" ]; then pass "$GROUP_B NOT auto-deleted (WS1 still in sway)"; else fail "$GROUP_B NOT auto-deleted (count=$GB_STILL)"; fi
...
# --- 15. Switch to Group B then back (NOW auto-delete, WS1 gone from sway) ---
run "Switch to Group B (should still exist)" \
...
if [ "$GB_GONE" = "0" ]; then pass "$GROUP_B auto-deleted (WS1 gone from sway)"; else fail "$GROUP_B auto-deleted (count=$GB_GONE)"; fi
